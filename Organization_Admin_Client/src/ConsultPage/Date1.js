...
const CalendarComponent = ({ cb }) => {
  ...
  // const [organizationName, setOrganizationName] = useState(getLoginCredentials[0].organization_name);
  ...
  const handleDateChange = async (date) => {
    ...
    try {
      ...
      const response = await axios.get(`${baseApiUrl}/consult_cancel_date_filter?organization_name=${selectOrganization}`, {
        params: {
  ...
  return (
    <div ref={calendarRef} style={{ position: 'relative', width: '200px', borderRadius: '15px' }}>
      <input
        ...
        placeholder='Jul 16, 2024 - Jul 27, 2024'
        readOnly
        style={{width: '200px',  border:'1px solid #ccc', fontSize: '12px', paddingRight: '15px', height: '30px', paddingLeft: '27px', marginLeft: '60px', marginBottom: '140px', textOverflow: 'ellipsis' }}
      />
      <FaAngleDown
