...
import CalendarComponent from './Date1.js';
...
const CombinedComponent = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${baseApiUrl}/get_consulting_api_b?organization_name=${selectOrganization}`);
  ...
  const toggleChildTable = (consult_id) => {
    setExpandedRow(prevState => prevState === consult_id ? null : consult_id);
  };
        ...
        {expandedRow === record.consult_id && getMedicineDetails.filter(med => med.consult_id === record.consult_id).map((childRecord, childIndex) => (
          <React.Fragment key={`child-${childIndex}`}>
            <tr className='table-row-consult' style={{ border: 'none' }}>
              ...
              />
              {/* <img src={cancel} className='table-row-pdf' alt='cancel' /> */}
